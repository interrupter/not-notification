...
export default class NotificationActionInbox {
	static async run(controller) {
		try {
			...
			controller.ui[ACTION] = new notTable(NotificationActionInbox.prepareOptions(controller));
	...
	static prepareOptions(controller) {
		const DEFAULT_OPTIONS_TABLE = {
			...
			fields: [{
				...
				preprocessor: (value) => {
					return [{
						action: controller.goDetails.bind(controller, value),
						title: 'not-notification:actionDetails',
						size: 'small'
					},
					{
						action: controller.goDelete.bind(controller, value),
						color: 'danger',
