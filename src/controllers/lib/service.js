import { Frame, say } from "not-bulma";
...
class nsNotification {
                    ...
                    if (res.result) {
                        ...
                        setTimeout(() => {
                            this.app.emit(`tag-${SECTION_ID}:update`, {
                                title:
                                    res.result.freshCount > 0
                                        ? res.result.freshCount
                                        : false,
                            });
                        }, 1000);
                    }
    ...
    createMenuItems({ list, count }) {
        ...
        items.push(this.createMenuBreak());
    ...
    getTimeDiff({ createdAt }) {
        ...
        let unit;
        if (sec < 60) {
            unit = this.declOfNum(sec, [
                "not-notification:second-1",
                "not-notification:second-2",
                "not-notification:second-3",
            ]);
            return say("not-notification:timeAgo", {
                time: sec,
                unit: say(unit),
            });
        } else if (sec < 3600) {
            let min = Math.floor(sec / 60);
            unit = this.declOfNum(min, [
